...
public class DummyData {
    ...
    public static List<OwnedCard> createPhysicalCardDummyList(int n) {
        ...
        for (int i = 0; i < n; i++)
            list.add(new OwnedCard( (i + 3000), Grade.getRandomGrade(), CardsmuleGame.randomGame(),  " " , "This is a valid description."));
        return list;
    }
    public static List<OwnedCardFetched> createPhysicalCardWithNameDummyList(int n) {
        return createPhysicalCardDummyList(n).stream().map(pCard -> new OwnedCardFetched(pCard, "test")).collect(Collectors.toList());
    }
}